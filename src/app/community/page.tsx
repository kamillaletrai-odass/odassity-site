...
export default function CommunityPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      ...
      <h1 className="mt-3 font-display text-hero leading-[0.98] text-ink italic">
      ...
      <div className="mt-16 flex flex-col gap-10">
        {authors.map((author) => {
          ...
          return (
            <div
              key={author.id}
              className="rounded-3xl border border-ink-faint bg-lavender-dim p-8 sm:p-10"
            >
              <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-lg text-ink-dim">{author.bio}</p>
                </div>
              </div>
                    ...
                    {pieces.map((piece) => (
                      <li key={piece.slug}>
                        <Link
                          ...
                          className="group flex items-center gap-3 rounded-xl bg-cloud/70 px-4 py-3 transition-colors hover:bg-cloud"
                        >
                          <span className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-pink">
                            {LENSES[piece.lens].label}
                          </span>
                          <span className="font-display text-ink group-hover:underline">
                            {piece.title}
                          </span>
                        </Link>
                      </li>
        ...
        <Link
          href="/write"
          className="flex flex-col items-start gap-2 rounded-3xl border border-dashed border-ink-faint p-8 transition-colors hover:border-pink/60 sm:p-10"
        >
          <p className="font-display text-2xl text-ink">
